...
function Product() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('https://localhost:7104/api/Product');
                ...
                <div className="bg-white shadow-lg border rounded-2xl p-6 cursor-pointer hover:transform hover:scale-105 transition-all duration-300 relative hover:shadow-2xl">
                  <div className="w-5/6 h-[210px] overflow-hidden mx-auto aspect-w-16 aspect-h-8 md:mb-2 mb-4">
                    <div className="block h-full w-full transition-transform duration-300">
                      <img src={`https://localhost:7104/images/${product.image}`} className="h-full w-full object-cover rounded-xl" alt={product.name} />
                    </div>
                  </div>
                  <div className="mt-4">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <div className="flex items-center space-x-2">
                        <span className="text-lg font-bold text-blue-500">{product.price.toLocaleString()} VND</span>
                        {product.originalPrice && (
                          <span className="text-sm text-gray-500 line-through">{product.originalPrice.toLocaleString()} VND</span>
                        )}
                      </div>
                      <button 
